...
export default function BentoGridServices({ businessName, niche, designType }: SectionProps) {
  ...
  const items = [
    ...
    { icon: FaSmile,    title: "Veneers",      desc: "Porcelain perfection", image: s[(3 + offset) % s.length].image },
    { icon: FaBroadcastTower, title: "Orthodontics", desc: "Invisalign & braces", image: s[(4 + offset) % s.length].image },
    { icon: FaShieldAlt, title: "Periodontics", desc: "Gum health experts", image: s[(5 + offset) % s.length].image },
  ];
    ...
    <section id="services" className="py-24 px-6" style={{ backgroundColor: "var(--secondary)" }}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 auto-rows-[200px]">
          ...
          >
            <img src={s[(1 + offset) % s.length].image} className="absolute inset-0 w-full h-full object-cover opacity-50 group-hover:scale-110 transition-transform duration-700" alt="Implants" />
            <FaTooth className="text-5xl text-white/60 relative z-10" />
            <div className="relative z-10">
              <h3 className="font-bold text-2xl mb-2" style={{ fontFamily: "var(--font-heading)" }}>Implants &amp; Restoration</h3>
              <p className="text-white font-medium text-sm">Permanent solutions that last a lifetime.</p>
            </div>
          </div>
          {items.map((item, i) => (
            <div
            ...
            >
              <img src={item.image} className="absolute inset-0 w-full h-full object-cover opacity-20 group-hover:opacity-40 transition-opacity duration-500" alt={item.title} />
